...
let texture_ChekkarBoard = null;
let sampler_ChekkarBoard = null;
let bindGroup_Texture_And_Sampler = null;

var chekkarBoardWidth = 64;
var chekkarBoardHeight = 64;
var checkImage = new Uint8ClampedArray(
    chekkarBoardWidth * chekkarBoardHeight * 4
);
...
function onDeviceLost(info)
{
    ...
    depthTexture = null;
    texture_ChekkarBoard = null;
    sampler_ChekkarBoard = null;
...
async function initialise() 
{
   ...
   // declare Texture array for rectangle
    ...
    texture_ChekkarBoard = await loadTexture();

    if(texture_ChekkarBoard == null)
    {
        console.log("initialise() : texture_ChekkarBoard is not supported by your browser.\n");
        throw Error("initialise() : texture_ChekkarBoard is not supported by your browser.\n");
    }
    else
    {
        console.log("initialise() : texture_ChekkarBoard is supported by your browser.\n");
    }
    ...
    sampler_ChekkarBoard = Device.createSampler(samplerDescriptor);

    if(sampler_ChekkarBoard == null)
    {
        console.log("initialise() : sampler_ChekkarBoard is not supported by your browser.\n");
        throw Error("initialise() : sampler_ChekkarBoard is not supported by your browser.\n");
    }
    else
    {
        console.log("initialise() : sampler_ChekkarBoard is supported by your browser.\n"); 
    }

    // create texure and sampler bind group
    bindGroup_Texture_And_Sampler = createBindGroupForTextureAndSampler(
        0,
        texture_ChekkarBoard,
        1,
        sampler_ChekkarBoard,
        bindGroupLayout_Texture_And_Sampler
    );
...
async function loadTexture()
{
    ...
    const imageData = new ImageData(pixels, chekkarBoardWidth, chekkarBoardHeight);
...
function makeCheckImage()
{
	...
	// code
	for (i = 0; i < chekkarBoardHeight; i++)
	{
		for (j = 0; j < chekkarBoardWidth; j++)
		{
			c = (((i & 0x8) == 0) ^ ((j & 0x8) == 0)) * 255;
            let index = (i * chekkarBoardWidth + j) * 4;
            checkImage[index] = c;
