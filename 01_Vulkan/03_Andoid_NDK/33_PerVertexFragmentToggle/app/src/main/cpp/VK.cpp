...
VkShaderModule vkShaderModule_vertex_shader_PF = VK_NULL_HANDLE;   // <-- You must add this
...
VkResult rebuildCommandBuffersIfReady(void)
{
    ...
    {
         __android_log_print(ANDROID_LOG_INFO, "PRS:", "rebuildCommandBuffersIfReady(): buildCommandBuffers() failed (%d)\n", vkresult);
    }
    else
    {
         __android_log_print(ANDROID_LOG_INFO, "PRS:", "rebuildCommandBuffersIfReady(): command buffers re-recorded for shader '%c'\n", choosenShader);
    }
                ...
                case AMOTION_EVENT_ACTION_DOWN:
                {
                   ...
                   if(bPendingSingleTap == true)
                   {
						...
						if(timeSinceLastTap <= 300 && distance <= 100.0f)
						{
							...
							__android_log_print(ANDROID_LOG_INFO,"PRS:","Double Tap Detected : choosenShader = %c\n",choosenShader);
						}
...
VkResult initialise(void)
{
	...
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "initialise() : createIndexBuffer() function failed (%d)\n", vkresult);
		return(vkresult);
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "initialise() : createIndexBuffer() succeeded\n");
	}
...
void uninitialise(void)
{
	...
	if (vkDevice)
	{
		...
		if (vkPipeline_PV)
		{
			vkDestroyPipeline(vkDevice, vkPipeline_PV, NULL);
			vkPipeline_PV = VK_NULL_HANDLE;
			 __android_log_print(ANDROID_LOG_INFO, "PRS:", "\nFree vkPipeline_PV: Pipeline freed successfully.\n");
		}
		...
		if (vkPipeline_PF)
		{
			vkDestroyPipeline(vkDevice, vkPipeline_PF, NULL);
			vkPipeline_PF = VK_NULL_HANDLE;
			 __android_log_print(ANDROID_LOG_INFO, "PRS:", "\nFree vkPipeline_PF: Pipeline freed successfully.\n");
		}
		...
		if (vkShaderModule_fragment_shader_PV)
		{
			...
			vkShaderModule_fragment_shader_PV = NULL;
			 __android_log_print(ANDROID_LOG_INFO, "PRS:", "\nFree vkShaderModule_fragment_shader_PV freed\n");
		}

		if (vkShaderModule_vertex_shader_PV)
		{
			vkDestroyShaderModule(vkDevice, vkShaderModule_vertex_shader_PV, NULL);
			vkShaderModule_vertex_shader_PV = NULL;
			 __android_log_print(ANDROID_LOG_INFO, "PRS:", "\nFree vkShaderModule_vertex_shader_PV freed\n");
		}

		if (vkShaderModule_fragment_shader_PF)
		{
			vkDestroyShaderModule(vkDevice, vkShaderModule_fragment_shader_PF, NULL);
			vkShaderModule_fragment_shader_PF = NULL;
			 __android_log_print(ANDROID_LOG_INFO, "PRS:", "\nFree vkShaderModule_fragment_shader_PF freed\n");
		}

		if (vkShaderModule_vertex_shader_PF)
		{
			vkDestroyShaderModule(vkDevice, vkShaderModule_vertex_shader_PF, NULL);
			vkShaderModule_vertex_shader_PF = NULL;
			 __android_log_print(ANDROID_LOG_INFO, "PRS:", "\nFree vkShaderModule_vertex_shader_PF freed\n");
		}
...
VkResult createVertexBuffer(void)
{
	...
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkCreateBuffer() function failed for normal Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkCreateBuffer() succeeded for normal\n");
	}
	...
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkAllocateMemory() function failed for normal Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkAllocateMemory() succeeded for normal\n");
	}

	vkresult = vkBindBufferMemory(vkDevice, vertexData_Normal.vkBuffer, vertexData_Normal.vkDeviceMemory, 0);
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkBindBufferMemory() function failed for normal Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkBindBufferMemory() succeeded for normal\n");
	}
	...
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkMapMemory() function failed for normal Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkMapMemory() succeeded for normal\n");
	}
	...
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkCreateBuffer() function failed for texcoord Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkCreateBuffer() succeeded for texcoord\n");
	}
	...
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkAllocateMemory() function failed for texcoord Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkAllocateMemory() succeeded for texcoord\n");
	}

	vkresult = vkBindBufferMemory(vkDevice, vertexData_Texcoord.vkBuffer, vertexData_Texcoord.vkDeviceMemory, 0);
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkBindBufferMemory() function failed for texcoord Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkBindBufferMemory() succeeded for texcoord\n");
	}
	...
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkMapMemory() function failed for texcoord Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkMapMemory() succeeded for texcoord\n");
	}
...
VkResult createIndexBuffer(void)
{
	...
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkCreateBuffer() function failed for position index buffer Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkCreateBuffer() succeeded position index buffer \n");
	}
	...
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkAllocateMemory() function failed for position index buffer Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkAllocateMemory() succeeded for position index buffer \n");
	}

	vkresult = vkBindBufferMemory(vkDevice, vertexData_Index.vkBuffer, vertexData_Index.vkDeviceMemory, 0);
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkBindBufferMemory() function failed for position index buffer Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkBindBufferMemory() succeeded for position index buffer\n");
	}
	...
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkMapMemory() function failed for position index buffer Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkMapMemory() succeeded for position index buffer\n");
	}
...
VkResult createShaders(void)
{
	...
	shaders[4] = {
        { "Shader_PV.vert.spv", &vkShaderModule_vertex_shader_PV },
        { "Shader_PV.frag.spv", &vkShaderModule_fragment_shader_PV },
        { "Shader_PF.vert.spv", &vkShaderModule_vertex_shader_PF },
        { "Shader_PF.frag.spv", &vkShaderModule_fragment_shader_PF }
    };

    for (int i = 0; i < 4; i++)
    {
        asset = AAssetManager_open(androidAssetManager, shaders[i].filename, AASSET_MODE_STREAMING);
        if (asset == NULL)
        {
            __android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : Failed to open %s\n", shaders[i].filename);
            return VK_ERROR_INITIALIZATION_FAILED;
        }
        ...
        if (size == 0)
        {
            __android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : Shader file %s has size 0\n", shaders[i].filename);
            AAsset_close(asset);
            return VK_ERROR_INITIALIZATION_FAILED;
        }
        ...
        if (retVal != size)
        {
            __android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : Failed to read %s\n", shaders[i].filename);
            free(shaderData);
            return VK_ERROR_INITIALIZATION_FAILED;
        }
        ...
        vkresult = vkCreateShaderModule(vkDevice, &vkShaderModuleCreateInfo, NULL, shaders[i].module);
        if (vkresult != VK_SUCCESS)
        {
            __android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : vkCreateShaderModule failed for %s, Error: %d\n", shaders[i].filename, vkresult);
            free(shaderData);
            return vkresult;
        }
        else
        {
            __android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : Shader module created successfully for %s\n", shaders[i].filename);
        }
...
VkResult createPipline(void)
{
	...
	// --- Shader stage arrays for PV and PF ---
	VkPipelineShaderStageCreateInfo vkPipelineShaderStageCreateInfo_Array_PV[2];
	VkPipelineShaderStageCreateInfo vkPipelineShaderStageCreateInfo_Array_PF[2];

	memset((void*)vkPipelineShaderStageCreateInfo_Array_PV, 0, sizeof(vkPipelineShaderStageCreateInfo_Array_PV));
	memset((void*)vkPipelineShaderStageCreateInfo_Array_PF, 0, sizeof(vkPipelineShaderStageCreateInfo_Array_PF));

	/* Per-Vertex pipeline: vertex = per-vertex lighting, fragment = passthrough (outputs interpolated color) */
	// PV - vertex
	vkPipelineShaderStageCreateInfo_Array_PV[0].sType  = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
	vkPipelineShaderStageCreateInfo_Array_PV[0].pNext  = NULL;
	vkPipelineShaderStageCreateInfo_Array_PV[0].flags  = 0;
	vkPipelineShaderStageCreateInfo_Array_PV[0].stage  = VK_SHADER_STAGE_VERTEX_BIT;
	vkPipelineShaderStageCreateInfo_Array_PV[0].module = vkShaderModule_vertex_shader_PV;
	vkPipelineShaderStageCreateInfo_Array_PV[0].pName  = "main";
	vkPipelineShaderStageCreateInfo_Array_PV[0].pSpecializationInfo = NULL;

	// PV - fragment
	vkPipelineShaderStageCreateInfo_Array_PV[1].sType  = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
	vkPipelineShaderStageCreateInfo_Array_PV[1].pNext  = NULL;
	vkPipelineShaderStageCreateInfo_Array_PV[1].flags  = 0;
	vkPipelineShaderStageCreateInfo_Array_PV[1].stage  = VK_SHADER_STAGE_FRAGMENT_BIT;
	vkPipelineShaderStageCreateInfo_Array_PV[1].module = vkShaderModule_fragment_shader_PV;
	vkPipelineShaderStageCreateInfo_Array_PV[1].pName  = "main";
	vkPipelineShaderStageCreateInfo_Array_PV[1].pSpecializationInfo = NULL;

	/* Per-Fragment pipeline: vertex = passthrough (position + normal), fragment = per-fragment lighting */
	// PF - vertex
	vkPipelineShaderStageCreateInfo_Array_PF[0].sType  = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
	vkPipelineShaderStageCreateInfo_Array_PF[0].pNext  = NULL;
	vkPipelineShaderStageCreateInfo_Array_PF[0].flags  = 0;
	vkPipelineShaderStageCreateInfo_Array_PF[0].stage  = VK_SHADER_STAGE_VERTEX_BIT;
	vkPipelineShaderStageCreateInfo_Array_PF[0].module = vkShaderModule_vertex_shader_PF;
	vkPipelineShaderStageCreateInfo_Array_PF[0].pName  = "main";
	vkPipelineShaderStageCreateInfo_Array_PF[0].pSpecializationInfo = NULL;

	// PF - fragment
	vkPipelineShaderStageCreateInfo_Array_PF[1].sType  = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
	vkPipelineShaderStageCreateInfo_Array_PF[1].pNext  = NULL;
	vkPipelineShaderStageCreateInfo_Array_PF[1].flags  = 0;
	vkPipelineShaderStageCreateInfo_Array_PF[1].stage  = VK_SHADER_STAGE_FRAGMENT_BIT;
	vkPipelineShaderStageCreateInfo_Array_PF[1].module = vkShaderModule_fragment_shader_PF;
	vkPipelineShaderStageCreateInfo_Array_PF[1].pName  = "main";
	vkPipelineShaderStageCreateInfo_Array_PF[1].pSpecializationInfo = NULL;

	// --- Create two pipelines using the same VkGraphicsPipelineCreateInfo skeleton ---
	...
	/* Reusable pipeline create info (fill the fixed-function pointers once) */
	...
	/* --- Create Per-Vertex pipeline --- */
	vkGraphicsPipelineCreateInfo.stageCount = 2;
	vkGraphicsPipelineCreateInfo.pStages = vkPipelineShaderStageCreateInfo_Array_PV;

	vkresult = vkCreateGraphicsPipelines(vkDevice, vkPipelineCache, 1, &vkGraphicsPipelineCreateInfo, NULL, &vkPipeline_PV);
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createPipeline() : Per-Vertex vkCreateGraphicsPipelines() failed. Error Code: (%d)\n", vkresult);
		// handle failure (cleanup, return error code, etc.)
		return vkresult;
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createPipeline() : Per-Vertex pipeline created successfully.\n");
	}

	/* --- Create Per-Fragment pipeline --- */
	vkGraphicsPipelineCreateInfo.stageCount = 2;
	vkGraphicsPipelineCreateInfo.pStages = vkPipelineShaderStageCreateInfo_Array_PF;

	vkresult = vkCreateGraphicsPipelines(vkDevice, vkPipelineCache, 1, &vkGraphicsPipelineCreateInfo, NULL, &vkPipeline_PF);
	if (vkresult != VK_SUCCESS)
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createPipeline() : Per-Fragment vkCreateGraphicsPipelines() failed. Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		 __android_log_print(ANDROID_LOG_INFO, "PRS:", "createPipeline() : Per-Fragment pipeline created successfully.\n");
	}
	...
	for (uint32_t i = 0; i < swapchainImageCount; i++)
	{
		...
		if (vkresult != VK_SUCCESS)
		{
			...
			return (vkresult);
			 ...
			 __android_log_print(ANDROID_LOG_INFO, "PRS:", "buildCommandBuffers() : vkResetCommandBuffer() successed at index[%d].\n", i);
		...
		if (vkresult != VK_SUCCESS)
		{
			...
			return (vkresult);
			 ...
			 __android_log_print(ANDROID_LOG_INFO, "PRS:", "buildCommandBuffers() : vkBeginCommandBuffer() successed at index[%d].\n", i);
		...
		if (vkresult != VK_SUCCESS)
		{
			...
			return (vkresult);
			 ...
			 __android_log_print(ANDROID_LOG_INFO, "PRS:", "buildCommandBuffers() : vkEndCommandBuffer() successed at index[%d].\n", i);
	}
