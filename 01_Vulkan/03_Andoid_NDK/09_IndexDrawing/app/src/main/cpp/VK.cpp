...
VertexData vertex_Position_index;
...
void uninitialise(void)
{
	...
	if (vkDevice)
	{
		...
		if (vertex_Position_index.vkDeviceMemory)
		{
			vkFreeMemory(vkDevice, vertex_Position_index.vkDeviceMemory, NULL);
			vertex_Position_index.vkDeviceMemory = VK_NULL_HANDLE;
			__android_log_print(ANDROID_LOG_INFO, "PRS:", "\nFreed vertex_Position_index.vkDeviceMemory \n");
		}

		if (vertex_Position_index.vkBuffer)
		{
			vkDestroyBuffer(vkDevice, vertex_Position_index.vkBuffer, NULL);
			vertex_Position_index.vkBuffer = VK_NULL_HANDLE;
			__android_log_print(ANDROID_LOG_INFO, "PRS:", "\nFree vertex_Position_index.vkBuffer freed\n");
		}
...
VkResult createVertexBuffer(void)
{
	...
	memset((void*)&vertex_Position_index, 0, sizeof(VertexData));
	...
	vkresult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vertex_Position_index.vkBuffer);
	if (vkresult != VK_SUCCESS)
	{
	...
	vkGetBufferMemoryRequirements(vkDevice, vertex_Position_index.vkBuffer, &vkMemoryRequirements);
	...
	vkresult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertex_Position_index.vkDeviceMemory);
	if (vkresult != VK_SUCCESS)
	{
	...
	vkresult = vkBindBufferMemory(vkDevice, vertex_Position_index.vkBuffer, vertex_Position_index.vkDeviceMemory, 0);
	if (vkresult != VK_SUCCESS)
	{
	...
	vkresult = vkMapMemory(vkDevice, vertex_Position_index.vkDeviceMemory, 0, vkMemoryAllocateInfo.allocationSize, 0, &data);
	if (vkresult != VK_SUCCESS)
	{
	...
	vkUnmapMemory(vkDevice, vertex_Position_index.vkDeviceMemory);
...
VkResult buildCommandBuffers(void)
{
	...
	for (uint32_t i = 0; i < swapchainImageCount; i++)
	{
		...
		vkCmdBindIndexBuffer(vkCommandBuffer_Array[i], vertex_Position_index.vkBuffer, 0, VK_INDEX_TYPE_UINT32); // UINT32 is matching with our indices array

		// Here we should call Vulkan drawing functions
		
		// second paramter correscponds to second paramter of glDrawElement
