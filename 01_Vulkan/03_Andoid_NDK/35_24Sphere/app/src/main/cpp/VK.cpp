...
#define _USE_MATH_DEFINES 1 // for M_PI
...
VkResult updateUniformbuffer(void)
{
	...
	if(gi_pressed_key == 1)
	{
		...
		myUniformData.lightPosition[1] = 45.0f * sin(lightAngle);
		myUniformData.lightPosition[2] = 45.0f * cos(lightAngle);
		myUniformData.lightPosition[3] = 1.0f;
	}

	else if (gi_pressed_key == 2)
	{
		myUniformData.lightPosition[0] = 45.0f * sin(lightAngle);
	...
	{
		myUniformData.lightPosition[0] = 45.0f * sin(lightAngle);
		myUniformData.lightPosition[1] = 45.0f * cos(lightAngle);
		myUniformData.lightPosition[2] = 0.0f;
		myUniformData.lightPosition[3] = 1.0f;
	}

	// update light angle
	if (gi_pressed_key == 1 || gi_pressed_key == 2 || gi_pressed_key == 3)
	{
		lightAngle = lightAngle + 0.03f;

		if (lightAngle > float(M_PI) * 2.0f) // full circle in radians
		{
			lightAngle -= float(M_PI) * 2.0f;
		}
	}
...
VkResult createPiplineLayout(void)
{
	...
	// initialize push constant related structure
...
VkResult createPipline(void)
{
	...
	// FOR THIS APPLICATION OUR VIEWPORT IS "DYNAMIC" HENCE VIEWPORT INITIALIZATION WILL REMOVED FROM HERE STILL WE ARE GOING TO USE VIEWPORT COUUNT HERE
...
VkResult buildCommandBuffers(void)
{
	...
	for (uint32_t i = 0; i < swapchainImageCount; i++)
	{
		...
		vkCmdPushConstants(vkCommandBuffer_Array[i], vkPipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(PushConstantData_Material), &pushConstantData_Material);
