...
{
	glm::mat4 mvpMatrix;
	glm::vec4 No_Of_LineSegments;
	glm::vec4 No_Of_LineStripes;
	glm::vec4 Line_Color;
};

unsigned int uiNumberOfLinesSegment = 1;
    ...
    while(1)
    {
        ...
        if(bPendingSingleTap == true)
        {
            ...
            if(now - pendingSingleTapTime >= 300) // 300 ms for single tap
			{
				...
				// Same as VK_UP key
				if(uiNumberOfLinesSegment < 30)
				{
					uiNumberOfLinesSegment++;
				}

				__android_log_print(ANDROID_LOG_INFO,"PRS:","Single Tap Detected : uiNumberOfLinesSegment = %u\n",uiNumberOfLinesSegment);
			}
                ...
                case AMOTION_EVENT_ACTION_DOWN:
                {
                   ...
                   if(bPendingSingleTap == true)
                   {
                        ...
                        if(timeSinceLastTap <= 300 && distance <= 100.0f)
						{
							...
							// Same as VK_DOWN key
							if(uiNumberOfLinesSegment > 1)
							{
								uiNumberOfLinesSegment--;
							}

							__android_log_print(ANDROID_LOG_INFO,"PRS:","Double Tap Detected : uiNumberOfLinesSegment = %u\n",uiNumberOfLinesSegment);
						}
...
void uninitialise(void)
{
	...
	if (vkDevice)
	{
		...
		if(vkShaderModule_tessellation_Evaluation_shader)
		{
			...
			vkShaderModule_tessellation_Evaluation_shader = NULL;
			__android_log_print(ANDROID_LOG_INFO, "PRS:", "\nFree vkShaderModule_tessellation_Evaluation_shader freed\n");
		}
		...
		if(vkShaderModule_tessellation_Control_shader)
		{
			...
			vkShaderModule_tessellation_Control_shader = NULL;
			__android_log_print(ANDROID_LOG_INFO, "PRS:", "\nFree vkShaderModule_tessellation_Control_shader freed\n");
		}
...
VkResult updateUniformbuffer(void)
{
	...
	modelMatrix = glm::translate(glm::mat4(1.0), glm::vec3(0.0f, 0.0f, -3.0f));
	...
	glm::mat4 projectionMatrix = perspectiveProjectionMatrix;

	myUniformData.mvpMatrix = projectionMatrix * viewMatrix * modelMatrix;


	// update line segment
	myUniformData.No_Of_LineSegments.x = uiNumberOfLinesSegment;
	//myUniformData.No_Of_LineStripes = (float)1;
	myUniformData.No_Of_LineStripes = glm::vec4(1.0f);

	// update line color
	myUniformData.Line_Color = glm::vec4(1.0f,1.0f,0.0f,1.0f);
...
VkResult createShaders(void)
{
	...
	if (asset == NULL)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : createShader failed to open tessellation control.spv file\n");
		vkresult = VK_ERROR_INITIALIZATION_FAILED;
		return vkresult;
	}
	else
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : createShader succeeded to open tessellation control.spv file\n");
	}
	...
	if (size == 0)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : createShader failed and give file size of tessellation control shader 0\n");
		vkresult = VK_ERROR_INITIALIZATION_FAILED;
		return vkresult;
	}
	...
	if (retVal != size)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : createShader failed to read tessellation control.spv file\n");
		vkresult = VK_ERROR_INITIALIZATION_FAILED;
		return vkresult;
	}
	else
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : createShader succeeded to read tessellation control.spv file\n");
	}
	...
	vkresult = vkCreateShaderModule(vkDevice, &vkShaderModuleCreateInfo, NULL, &vkShaderModule_tessellation_Control_shader);
	if (vkresult != VK_SUCCESS)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : vkCreateShaderModule() function failed. for tessellation control shader Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : vkCreateShaderModule() succeeded for tessellation control shader.\n");
	}
	...
	__android_log_print(ANDROID_LOG_INFO, "PRS:", "fragment Module tessellation control created\n");
	...
	if (asset == NULL)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : createShader failed to open tessellation Evaluation.spv file\n");
		vkresult = VK_ERROR_INITIALIZATION_FAILED;
		return vkresult;
	}
	else
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : createShader succeeded to open tessellation Evaluation.spv file\n");
	}
	...
	if (size == 0)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : createShader failed and give file size of tessellation Evaluation shader 0\n");
		vkresult = VK_ERROR_INITIALIZATION_FAILED;
		return vkresult;
	}
	...
	if (retVal != size)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : createShader failed to read tessellation Evaluation.spv file\n");
		vkresult = VK_ERROR_INITIALIZATION_FAILED;
		return vkresult;
	}
	else
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : createShader succeeded to read tessellation Evaluation.spv file\n");
	}
	...
	vkresult = vkCreateShaderModule(vkDevice, &vkShaderModuleCreateInfo, NULL, &vkShaderModule_tessellation_Evaluation_shader);
	if (vkresult != VK_SUCCESS)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : vkCreateShaderModule() function failed. for tessellation Evaluation shader Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : vkCreateShaderModule() succeeded for tessellation Evaluation shader.\n");
	}
	...
	__android_log_print(ANDROID_LOG_INFO, "PRS:", "fragment Module tessellation Evaluation created\n");
	...
	if (size == 0)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createShaders() : createShader failed and give file size of fragment shader 0\n");
		vkresult = VK_ERROR_INITIALIZATION_FAILED;
		return vkresult;
	}
...
VkResult createPipline(void)
{
	...
	vkVertexInputBindingDescription_Array[0].stride = sizeof(float) * 2; // we have only x , y gemortry now
	...
	vkPipelineTessellationStateCreateInfo.patchControlPoints = 4; // similar to 2nd parameter  of glpatchparamerti()
