		...
		if(bPendingSingleTap == true)
		{
			...
			if(now - pendingSingleTapTime >= 300) // 300 ms for single tap
			{
				...
				__android_log_print(
					ANDROID_LOG_INFO,
					"PRS:",
					"Single Tap Detected : gi_pressed_key = %d\n",
					gi_pressed_key
				);
			}
		}
...
VkResult display(void)
{
	...
	if(vkresult != VK_SUCCESS)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "display() : updateTexcoordBuffer() failed with error: %d\n", vkresult);
		return(vkresult);
	}
...
VkResult updateTexcoordBuffer(void)
{
    ...
    if (vkresult != VK_SUCCESS)
    {
        __android_log_print(ANDROID_LOG_INFO, "PRS:", "updateTexcoordBuffer() : vkMapMemory() failed. Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    ...
    __android_log_print(ANDROID_LOG_INFO, "PRS:", "updateTexcoordBuffer() : texcoords updated for key = %d\n", gi_pressed_key);
