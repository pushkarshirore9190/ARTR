...
VkResult createVertexBuffer(void)
{
	...
	//code
	VertexData vertexData_StaggingBuffer_Position;
	...
	memset((void*)&vertexData_StaggingBuffer_Position, 0, sizeof(VertexData));

	VkBufferCreateInfo vkBufferCreateInfo_StaggingBuffer;
	memset((void*)&vkBufferCreateInfo_StaggingBuffer, 0, sizeof(VkBufferCreateInfo));

	vkBufferCreateInfo_StaggingBuffer.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
	vkBufferCreateInfo_StaggingBuffer.pNext = NULL;
	vkBufferCreateInfo_StaggingBuffer.flags = 0;
	vkBufferCreateInfo_StaggingBuffer.size = sizeof(traingle_Position);
	vkBufferCreateInfo_StaggingBuffer.usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
	vkBufferCreateInfo_StaggingBuffer.sharingMode = VK_SHARING_MODE_EXCLUSIVE;

	vkresult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo_StaggingBuffer, NULL, &vertexData_StaggingBuffer_Position.vkBuffer);
	if (vkresult != VK_SUCCESS)
	{
	...
	vkGetBufferMemoryRequirements(vkDevice, vertexData_StaggingBuffer_Position.vkBuffer, &vkMemoryRequirements_StaggingBuffer);
	...
	vkresult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo_StaggingBuffer, NULL, &vertexData_StaggingBuffer_Position.vkDeviceMemory);
	if (vkresult != VK_SUCCESS)
	{
	...
	vkresult = vkBindBufferMemory(vkDevice, vertexData_StaggingBuffer_Position.vkBuffer, vertexData_StaggingBuffer_Position.vkDeviceMemory, 0);
	if (vkresult != VK_SUCCESS)
	{
	...
	vkresult = vkMapMemory(vkDevice, vertexData_StaggingBuffer_Position.vkDeviceMemory, 0, vkMemoryAllocateInfo_StaggingBuffer.allocationSize, 0, &data);
	if (vkresult != VK_SUCCESS)
	{
	...
	vkUnmapMemory(vkDevice, vertexData_StaggingBuffer_Position.vkDeviceMemory);
	...
	// No need fo unmap memory , unmap memory and memcpy
	...
	if (vkresult != VK_SUCCESS)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkAllocateCommandBuffers() function failed to make crreateCommandBuffer for ""BufferCopy"" Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkAllocateCommandBuffers() succeeded to crreateCommandBuffer for ""BufferCopy"" \n");
	}
	...
	if (vkresult != VK_SUCCESS)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkBeginCommandBuffer() function failed Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkBeginCommandBuffer() succeeded \n");
	}
	...
	vkBufferCopy.size = sizeof(traingle_Position);

	vkCmdCopyBuffer(vkCommandBuffer, vertexData_StaggingBuffer_Position.vkBuffer, vertexData_Position.vkBuffer, 1, &vkBufferCopy);
	if (vkresult != VK_SUCCESS)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkCmdCopyBuffer() function failed Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkCmdCopyBuffer() succeeded \n");
	}

	vkresult = vkEndCommandBuffer(vkCommandBuffer);
	if (vkresult != VK_SUCCESS)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkEndCommandBuffer() function failed Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkEndCommandBuffer() succeeded \n");
	}

	// now submit command buffer into queue
	...
	if (vkresult != VK_SUCCESS)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkQueueSubmit() function failed Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkQueueSubmit() succeeded \n");
	}

	vkresult = vkQueueWaitIdle(vkQueue);
	if (vkresult != VK_SUCCESS)
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkQueueWaitIdle() function failed Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "createVertexBuffer() : vkQueueWaitIdle() succeeded \n");
	}
	...
	if (vertexData_StaggingBuffer_Position.vkBuffer != VK_NULL_HANDLE) {
		vkDestroyBuffer(vkDevice, vertexData_StaggingBuffer_Position.vkBuffer, NULL);
		vertexData_StaggingBuffer_Position.vkBuffer = VK_NULL_HANDLE;
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "Local staging buffer vertexData_StaggingBuffer_Position.vkBuffer destroyed\n");
	}

	if (vertexData_StaggingBuffer_Position.vkDeviceMemory != VK_NULL_HANDLE) {
		vkFreeMemory(vkDevice, vertexData_StaggingBuffer_Position.vkDeviceMemory, NULL);
		vertexData_StaggingBuffer_Position.vkDeviceMemory = VK_NULL_HANDLE;
		__android_log_print(ANDROID_LOG_INFO, "PRS:", "Local staging buffer vertexData_StaggingBuffer_Position.vkDeviceMemory freed\n");
	}
...
;	vkDescriptorSetLayoutCreateInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
	...
	vkDescriptorSetLayoutCreateInfo.pBindings = NULL;  // pbinding array is actually array VkDiscriptorSetLayoutBinding having 5 members  1) uint32_t binding : an integer value where you want to bind descriptor set  
