...
VkResult resize(int width, int heigth)
{
	...
	if (vkPipeline_PF)
	{
		...
		fprintf(gpFile, "\n resize() : Free vkPipeline_PF freed\n");
	}
	...
	if (vkPipeline_PV)
	{
		...
		fprintf(gpFile, "\n resize() : Free vkPipeline_PV freed\n");
	}
...
VkResult createShaders(void)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        fp = fopen(shaders[i].filename, "rb");
        if (fp == NULL)
        {
            fprintf(gpFile, "createShaders() : Failed to open %s\n", shaders[i].filename);
            return VK_ERROR_INITIALIZATION_FAILED;
        }
        ...
        if (size == 0)
        {
            fprintf(gpFile, "createShaders() : Shader file %s has size 0\n", shaders[i].filename);
            fclose(fp);
            return VK_ERROR_INITIALIZATION_FAILED;
        }
        ...
        if (retVal != 1)
        {
            fprintf(gpFile, "createShaders() : Failed to read %s\n", shaders[i].filename);
            free(shaderData);
            return VK_ERROR_INITIALIZATION_FAILED;
        }
        ...
        if (vkresult != VK_SUCCESS)
        {
            fprintf(gpFile, "createShaders() : vkCreateShaderModule failed for %s, Error: %d\n", shaders[i].filename, vkresult);
            free(shaderData);
            return vkresult;
        }
        else
        {
            fprintf(gpFile, "createShaders() : Shader module created successfully for %s\n", shaders[i].filename);
        }
...
VkResult createPipline(void)
{
	...
	if (vkresult != VK_SUCCESS)
	{
		fprintf(gpFile, "createPipeline() : Per-Vertex vkCreateGraphicsPipelines() failed. Error Code: (%d)\n", vkresult);
		// handle failure (cleanup, return error code, etc.)
		return vkresult;
	}
	else
	{
		fprintf(gpFile, "createPipeline() : Per-Vertex pipeline created successfully.\n");
	}
	...
	if (vkresult != VK_SUCCESS)
	{
		fprintf(gpFile, "createPipeline() : Per-Fragment vkCreateGraphicsPipelines() failed. Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		fprintf(gpFile, "createPipeline() : Per-Fragment pipeline created successfully.\n");
	}
