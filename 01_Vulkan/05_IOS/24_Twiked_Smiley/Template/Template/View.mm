...
-(VkResult) render
{
    ...
    vkresult = [self updateTexcoordBuffer];
    if(vkresult != VK_SUCCESS)
    {
        printf( "render() : updateTexcoordBuffer() failed with error: %d\n", vkresult);
        return(vkresult);
    }
...
-(VkResult) updateTexcoordBuffer
{
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "updateTexcoordBuffer() : vkMapMemory() failed. Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    ...
    printf( "updateTexcoordBuffer() : texcoords updated for key = %d\n", gi_pressed_key);
