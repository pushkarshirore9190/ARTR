...
-(void) loadOBJFile
{
    ...
    NSString* objFileNameWithPath =
        [[appBundle resourcePath] stringByAppendingPathComponent:@"Model.obj"];

    const char* pszObjFileNameWithPath =
        [objFileNameWithPath cStringUsingEncoding:NSUTF8StringEncoding];

    std::ifstream dataIn;
    dataIn.open(pszObjFileNameWithPath);

    if (!dataIn.is_open())
    {
        printf( "loadOBJFile() : Failed to open Model.obj\n");
        exit(-1);
    }
