...
-(void)uninitialise
{
    ...
    if (vkDevice)
    {
        ...
        if (vertexData_PCNT.vkDeviceMemory)
        {
            ...
            printf( "\nFree vertexData_PCNT.vkDeviceMemory freed\n");
        ...
        if (vertexData_PCNT.vkBuffer)
        {
            ...
            printf( "\nFree vertexData_PCNT.vkBuffer freed\n");
...
-(VkResult) createVertexBuffer
{
    ...
    vkresult = vkMapMemory(vkDevice, vertexData_PCNT.vkDeviceMemory, 0, vkMemoryAllocateInfo.allocationSize, 0, &data);
    if (vkresult != VK_SUCCESS)
    {
...
-(VkResult) createTexture:(const char*)textureFileName
{
    ...
    if (fp == NULL)
    {
        printf( "createTexture() : fopen failed for reading texture file\n");
        vkresult = VK_ERROR_INITIALIZATION_FAILED;
        return vkresult;
    }
    else
    {
        printf( "createTexture() : fopen succeeded for texture file\n");
    }
    ...
    {
        printf( "createTexture() : stbi_load_from_file function failed\n");
        vkresult = VK_ERROR_INITIALIZATION_FAILED;
        return vkresult;
    }
...
-(VkResult) createPipline
{
    ...
    vkVertexInputBindingDescription_Array[0].stride = sizeof(float) * 11;
    ...
    vkVertexInputAttributeDescription_Array[0].offset = sizeof(float) * 0; // size of float * 0 * 0 = 0
    ...
    vkVertexInputAttributeDescription_Array[1].offset = sizeof(float) * 3 ; // size of float * 3 * 1 = 12
    ...
    vkVertexInputAttributeDescription_Array[2].offset = sizeof(float) * 6; // size of float * 3 * 2 = 24
    ...
    vkVertexInputAttributeDescription_Array[3].offset = sizeof(float) * 9; // size of float * 2 * 3 = 36
...
-(VkResult) buildCommandBuffers
{
    ...
    for (uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        VkDeviceSize vkDeviceSize_Offest_PCNT[1];
        memset((void*)vkDeviceSize_Offest_PCNT, 0, sizeof(VkDeviceSize) * _ARRAYSIZE(vkDeviceSize_Offest_PCNT));

        vkCmdBindVertexBuffers(vkCommandBuffer_Array[i], 0, 1, &vertexData_PCNT.vkBuffer, vkDeviceSize_Offest_PCNT);
