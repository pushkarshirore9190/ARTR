...
-(void)onDoubleTap:(UITapGestureRecognizer *)gestureRecognizer
{
    ...
    [self rebuildCommandBuffersIfReady];
}
...
-(VkResult) rebuildCommandBuffersIfReady
{
    VkResult vkresult = VK_SUCCESS;
    ...
    displayLink.paused = YES;          // was CVDisplayLinkStop(displayLink);
    ...
    if (vkresult != VK_SUCCESS)
        printf("rebuildCommandBuffersIfReady(): buildCommandBuffers() failed (%d)\n", vkresult);
    else
        printf("rebuildCommandBuffersIfReady(): command buffers re-recorded for shader '%c'\n", choosenShader);

    displayLink.paused = NO;           // was CVDisplayLinkStart(displayLink);
...
-(VkResult) initialise
{
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "initialise() : createIndexBuffer() function failed (%d)\n", vkresult);
        return(vkresult);
    }
    else
    {
        printf( "initialise() : createIndexBuffer() succeeded\n");
    }
...
-(VkResult) createVertexBuffer
{
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createVertexBuffer() : vkCreateBuffer() function failed for normal Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createVertexBuffer() : vkCreateBuffer() succeeded for normal\n");
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createVertexBuffer() : vkAllocateMemory() function failed for normal Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createVertexBuffer() : vkAllocateMemory() succeeded for normal\n");
    }

    vkresult = vkBindBufferMemory(vkDevice, vertexData_Normal.vkBuffer, vertexData_Normal.vkDeviceMemory, 0);
    if (vkresult != VK_SUCCESS)
    {
        printf( "createVertexBuffer() : vkBindBufferMemory() function failed for normal Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createVertexBuffer() : vkBindBufferMemory() succeeded for normal\n");
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createVertexBuffer() : vkMapMemory() function failed for normal Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createVertexBuffer() : vkMapMemory() succeeded for normal\n");
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createVertexBuffer() : vkCreateBuffer() function failed for texcoord Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createVertexBuffer() : vkCreateBuffer() succeeded for texcoord\n");
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createVertexBuffer() : vkAllocateMemory() function failed for texcoord Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createVertexBuffer() : vkAllocateMemory() succeeded for texcoord\n");
    }

    vkresult = vkBindBufferMemory(vkDevice, vertexData_Texcoord.vkBuffer, vertexData_Texcoord.vkDeviceMemory, 0);
    if (vkresult != VK_SUCCESS)
    {
        printf( "createVertexBuffer() : vkBindBufferMemory() function failed for texcoord Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createVertexBuffer() : vkBindBufferMemory() succeeded for texcoord\n");
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createVertexBuffer() : vkMapMemory() function failed for texcoord Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createVertexBuffer() : vkMapMemory() succeeded for texcoord\n");
    }
...
-(VkResult) createIndexBuffer
{
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createVertexBuffer() : vkCreateBuffer() function failed for position index buffer Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createVertexBuffer() : vkCreateBuffer() succeeded position index buffer \n");
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createVertexBuffer() : vkAllocateMemory() function failed for position index buffer Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createVertexBuffer() : vkAllocateMemory() succeeded for position index buffer \n");
    }

    vkresult = vkBindBufferMemory(vkDevice, vertexData_Index.vkBuffer, vertexData_Index.vkDeviceMemory, 0);
    if (vkresult != VK_SUCCESS)
    {
        printf( "createVertexBuffer() : vkBindBufferMemory() function failed for position index buffer Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createVertexBuffer() : vkBindBufferMemory() succeeded for position index buffer\n");
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createVertexBuffer() : vkMapMemory() function failed for position index buffer Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createVertexBuffer() : vkMapMemory() succeeded for position index buffer\n");
    }
...
-(VkResult) createShaders
{
    ...
    for (int i = 0; i < 4; i++)
    {
        NSString* shaderFileNameWithPath =
            [[appBundle resourcePath] stringByAppendingPathComponent:@(shaders[i].filename)];

        const char* pszShaderFileNameWithPath =
            [shaderFileNameWithPath cStringUsingEncoding:NSUTF8StringEncoding];

        FILE* fp = fopen(pszShaderFileNameWithPath, "rb");

        if (fp == NULL)
        {
            printf( "createShaders() : Failed to open %s\n", shaders[i].filename);
            return VK_ERROR_INITIALIZATION_FAILED;
        }
        else
        {
            printf( "createShaders() : Successfully opened %s\n", shaders[i].filename);
        }
        ...
        {
            fclose(fp);
            printf( "createShaders() : %s size is 0\n", shaders[i].filename);
            return VK_ERROR_INITIALIZATION_FAILED;
        }
        ...
        {
            free(shaderData);
            printf( "createShaders() : Failed to read %s\n", shaders[i].filename);
            return VK_ERROR_INITIALIZATION_FAILED;
        }
        else
        {
            printf( "createShaders() : Successfully read %s\n", shaders[i].filename);
        }
        ...
        if (vkresult != VK_SUCCESS)
        {
            printf( "createShaders() : vkCreateShaderModule() failed for %s (%d)\n",
                   shaders[i].filename,
                   vkresult);
            return vkresult;
        }
        ...
        {
            printf( "createShaders() : Successfully created shader module for %s\n", shaders[i].filename);
        }
    }
...
-(VkResult) createPipline
{
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createPipeline() : Per-Vertex vkCreateGraphicsPipelines() failed. Error Code: (%d)\n", vkresult);
        // handle failure (cleanup, return error code, etc.)
        return vkresult;
    }
    else
    {
        printf( "createPipeline() : Per-Vertex pipeline created successfully.\n");
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createPipeline() : Per-Fragment vkCreateGraphicsPipelines() failed. Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createPipeline() : Per-Fragment pipeline created successfully.\n");
    }
...
-(VkResult) buildCommandBuffers
{
    ...
    for (uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        if (vkresult != VK_SUCCESS)
        {
            ...
            printf( "buildCommandBuffers() : vkResetCommandBuffer() successed at index[%d].\n", i);
        ...
        if (vkresult != VK_SUCCESS)
        {
            ...
            printf( "buildCommandBuffers() : vkBeginCommandBuffer() successed at index[%d].\n", i);
        ...
        if (vkresult != VK_SUCCESS)
        {
            ...
            printf( "buildCommandBuffers() : vkEndCommandBuffer() successed at index[%d].\n", i);
    }
