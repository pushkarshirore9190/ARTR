...
-(VkResult) initialise
{
    ...
    vkresult = [self createTexture:"Stone.png"
                        vkImage:&vkImage_Texture_Pyramid
                    vkDeviceMemory:&vkDeviceMemory_Texture_Pyramid
                    vkImageView:&vkImageView_Texture_Pyramid
                        vkSampler:&vkSampler_Texture_Pyramid];
    if (vkresult != VK_SUCCESS)
    {
        printf( "initialise() : createTexture() function failed for Pyramid (Stone.png). Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "initialise() : createTexture() succeeded for Pyramid (Stone.png)\n");
    }

    // Create Cube texture
    vkresult = [self createTexture:"Vijay_Kundali.png"
                        vkImage:&vkImage_Texture_Cube
                    vkDeviceMemory:&vkDeviceMemory_Texture_Cube
                    vkImageView:&vkImageView_Texture_Cube
                        vkSampler:&vkSampler_Texture_Cube];
    if (vkresult != VK_SUCCESS)
    {
        printf( "initialise() : createTexture() function failed for Cube (Vijay_Kundali.png). Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "initialise() : createTexture() succeeded for Cube (Vijay_Kundali.png)\n");
    }
...
-(void)uninitialise
{
    ...
    if (vkDevice)
    {
        ...
        if (vertexData_Texcoord_Cube.vkBuffer)
        {
            ...
            printf( "\vertexData_Texcoord_Cube.vkBuffer freed\n");
        ...
        if (vertexData_Texcoord_Pyramid.vkBuffer)
        {
            ...
            printf( "\nFree vertexData_Texcoord_Pyramid.vkBuffer freed\n");
...
-(VkResult) createVertexBuffer
{
    ...
    VkBool32 foundMatchingMemoryType_Vertex_Rect = VK_FALSE;
    ...
    for (uint32_t i = 0; i < vkPhysicalDeviceMemoryProperties.memoryTypeCount; i++)
    {
        if ((vkMemoryRequirements.memoryTypeBits & 1) == 1)
        {
            ...
            {
                vkMemoryAllocateInfo.memoryTypeIndex = i;
                foundMatchingMemoryType_Vertex_Rect = VK_TRUE;
                break;
            }
        }
    ...
    if (foundMatchingMemoryType_Vertex_Rect == VK_FALSE)
    {
        vkresult = VK_ERROR_OUT_OF_HOST_MEMORY;
        printf( "createVertexBuffer() : Failed to find suitable memory type for vertex buffer\n");
        return vkresult; // Return hardcoded error code
...
-(VkResult) createTexture:(const char*)textureFileName
                  vkImage:(VkImage*)out_vkImage
           vkDeviceMemory:(VkDeviceMemory*)out_vkDeviceMemory
              vkImageView:(VkImageView*)out_vkImageView
                vkSampler:(VkSampler*)out_vkSampler
{
    // Variable declaration
    VkResult vkresult = VK_SUCCESS;
    ...
    if (fp == NULL)
    {
        printf( "createTexture() : fopen failed for reading texture file %s\n", textureFileName);
        vkresult = VK_ERROR_INITIALIZATION_FAILED;
        return vkresult;
    }
    else
    {
        printf( "createTexture() : fopen succeeded for texture file %s\n", textureFileName);
    }
    ...
    {
        printf( "createTexture() : stbi_load_from_file function failed for %s\n", textureFileName);
        vkresult = VK_ERROR_INITIALIZATION_FAILED;
        return vkresult;
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createTexture() : vkMapMemory() function failed. Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createTexture() : vkMapMemory() succeeded.\n");
    }
    ...
    printf( "createTexture() : stbi_image_free() succeeded for image data\n");
    ...
    vkCmdCopyBufferToImage(VkCommandBuffer_Buffer_To_Image_Copy,
        vkBuffer_StaggingBuffer, *out_vkImage,
    ...
    vkImageViewCreateInfo.pNext = NULL;
    vkImageViewCreateInfo.flags = 0;
    vkImageViewCreateInfo.format = VK_FORMAT_R8G8B8A8_SRGB;   // sRGB fix from before

    // Compensate for MoltenVK/iOS mapping this format to a BGRA-ordered
    // Metal texture internally, which swaps R and B on sample.
    vkImageViewCreateInfo.components.r = VK_COMPONENT_SWIZZLE_B;
    vkImageViewCreateInfo.components.g = VK_COMPONENT_SWIZZLE_G;
    vkImageViewCreateInfo.components.b = VK_COMPONENT_SWIZZLE_R;
    vkImageViewCreateInfo.components.a = VK_COMPONENT_SWIZZLE_A;
    ...
    vkresult = vkCreateSampler(vkDevice, &vkSamplerCreateInfo, NULL, out_vkSampler);
    if (vkresult != VK_SUCCESS)
    {
        printf( "createTexture() : vkCreateSampler() function failed. Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
...
-(VkResult) createUniformBuffer
{
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createUniformBuffer() : vkCreateBuffer() function failed  for traingle Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createUniformBuffer() : vkCreateBuffer() succeeded  for traingle\n");
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createUniformBuffer() : vkAllocateMemory() function failed  for traingle Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createUniformBuffer() : vkAllocateMemory() succeeded  for traingle\n");
    }

    vkresult = vkBindBufferMemory(vkDevice, uniformData_Pyramid.vkBuffer, uniformData_Pyramid.vkDeviceMemory, 0);
    if (vkresult != VK_SUCCESS)
    {
        printf( "createUniformBuffer() : vkBindBufferMemory() function failed  for traingle Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createUniformBuffer() : vkBindBufferMemory() succeeded  for traingle\n");
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createUniformBuffer() : vkCreateBuffer() function failed  for reactangle Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createUniformBuffer() : vkCreateBuffer() succeeded  for reactangle\n");
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createUniformBuffer() : vkAllocateMemory() function failed  for reactangle Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createUniformBuffer() : vkAllocateMemory() succeeded  for reactangle\n");
    }

    vkresult = vkBindBufferMemory(vkDevice, uniformData_Cube.vkBuffer, uniformData_Cube.vkDeviceMemory, 0);
    if (vkresult != VK_SUCCESS)
    {
        printf( "createUniformBuffer() : vkBindBufferMemory() function failed  for reactangle Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createUniformBuffer() : vkBindBufferMemory() succeeded  for reactangle\n");
    }
...
-(VkResult) createDescriptorSet
{
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createDescriptorSet() : vkAllocateDescriptorSets() failed for Pyramid. Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createDescriptorSet() : vkAllocateDescriptorSets() succeeded for Pyramid.\n");
    }
    ...
    printf( "vkUpdateDescriptorSets() succeeded for Pyramid\n");
    ...
    if (vkresult != VK_SUCCESS)
    {
        printf( "createDescriptorSet() : vkAllocateDescriptorSets() failed for Cube. Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        printf( "createDescriptorSet() : vkAllocateDescriptorSets() succeeded for Cube.\n");
    }
    ...
    vkUpdateDescriptorSets(vkDevice, _ARRAYSIZE(vkWriteDescriptorSet_Array), vkWriteDescriptorSet_Array, 0, NULL);
    printf( "vkUpdateDescriptorSets() succeeded for Cube\n");
...
-(VkResult) createPipline
{
    ...
    vkPipelineColorBlendAttachmentState_Array[0].colorWriteMask =
