...
const char* enabledInstanceExtensionNames_array[4];  // VK_KHR_SURFACE_EXTENSION_NAME and VK_EXT_METAL_SURFACE_EXTENSION_NAME and VK_EXT_DEBUG_REPORT_EXTENSION_NAME
...
-(NSSize)windowWillResize:(NSWindow*)sender toSize:(NSSize)frameSize
{
    ...
    {
		[self resize:(int)frameSize.width :(int)frameSize.height];
    }
    return frameSize;
}
...
-(VkResult) initialise    
{
	...
	if (vkresult != VK_SUCCESS) {
		fprintf(gpFile, "initialise() : createDiscriptorSetLayout_fbo() failed (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
...
{	
    ...
    if (vkresult != VK_SUCCESS)
    {
        fprintf(gpFile, "resize() : createPiplineLayout() failed (%d)\n", vkresult);
        return(vkresult);
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        fprintf(gpFile, "resize() : createPipline() failed (%d)\n", vkresult);
        return(vkresult);
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        fprintf(gpFile, "resize() : vkCreateSemaphore backbuffer failed (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        fprintf(gpFile, "resize() : vkCreateSemaphore backbuffer recreated successfully\n");
    }
    return(vkresult);
}
...
-(VkResult) render
{
    ...
    {
        fprintf(gpFile, "render(): FBO initialisation yet not completed\n");
        return (VkResult)VK_FALSE;
    }
    if (bInitialised == NO)
    {
        fprintf(gpFile, "render(): initialisation yet not completed\n");
        return (VkResult)VK_FALSE;
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        fprintf(gpFile, "render() : FBO vkQueueSubmit failed: %d\n", vkresult);
        return(vkresult);
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        fprintf(gpFile, "render() : main vkQueueSubmit failed: %d\n", vkresult);
        return(vkresult);
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        ...
        {
            fprintf(gpFile, "render() : vkQueuePresentKHR failed: %d\n", vkresult);
            return(vkresult);
        }
    }
    vkDeviceWaitIdle(vkDevice);
    ...
    if(vkresult != VK_SUCCESS)
    {
        fprintf(gpFile, "render() : updateUniformbuffer() failed: %d\n", vkresult);
        return(vkresult);
    }
...
-(VkResult) fillExtensionNames
{
    ...
    {
        vkresult = VK_ERROR_INITIALIZATION_FAILED; // return hardcoded failure
        fprintf(gpFile, "fillExtensionNames() : VK_EXT_METAL_SURFACE_EXTENSION_NAME not found\n");
        return vkresult;
    }
    else
    {
        fprintf(gpFile, "fillExtensionNames() : VK_EXT_METAL_SURFACE_EXTENSION_NAME found\n");
    }
...
-(VkResult) getSupportedSurface
{
    ...
    if(vkresult != VK_SUCCESS)
    {
        fprintf(gpFile, "getSupportedSurface() : vkCreateMetalSurfaceEXT Failed\n");
        return vkresult;
    }
    else
    {
        fprintf(gpFile, "getSupportedSurface() : vkCreateMetalSurfaceEXT succeded \n");
    }
...
-(VkResult) resize_fbo : (int)fbo_width : (int)fbo_heigth
{
	...
	{
		vkDeviceWaitIdle(vkDevice);
		fprintf(gpFile, "resize_fbo() : vkDeviceWaitIdle Done\n");
	}
...
-(VkResult) createImagesAndImageViews_fbo
{
	...
	if (vkresult != VK_SUCCESS)
	{
		fprintf(gpFile, "createImagesAndImageViews_fbo() : vkCreateImageView() function failed for fbo image. Error Code: (%d)\n", vkresult);
		return vkresult;
	}
	else
	{
		fprintf(gpFile, "createImagesAndImageViews_fbo() : vkCreateImageView() succeeded for fbo image.\n");
	}
...
-(VkResult) createTexture_fbo:(const char*)textureFileName
{
    ...
    const char* pszTextureFileNameWithPath = [textureFileNameWithPath cStringUsingEncoding:NSUTF8StringEncoding];
    ...
    if (fp == NULL)
    {
        fprintf(gpFile, "createTexture() : fopen failed for reading texture file\n");
        vkresult = VK_ERROR_INITIALIZATION_FAILED;
        return vkresult;
    }
    else
    {
        fprintf(gpFile, "createTexture() : fopen succeeded for texture file\n");
    }
    ...
    {
        fprintf(gpFile, "createTexture() : stbi_load_from_file function failed\n");
        vkresult = VK_ERROR_INITIALIZATION_FAILED;
        return vkresult;
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        fprintf(gpFile, "createTexture() : vkMapMemory() function failed. Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
        fprintf(gpFile, "createTexture() : vkMapMemory() succeeded.\n");
    }
    ...
    if (vkresult != VK_SUCCESS)
    {
        fprintf(gpFile, "createTexture() : vkCreateSampler() function failed. Error Code: (%d)\n", vkresult);
        return vkresult;
    }
    else
    {
