...
-(VkResult) createVulkanDevice
{
	...
	// Enable only features supported by the device
...
-(VkResult) createShaders
{
	...
	if (fp == NULL)
	{
		fprintf(gpFile, "createShaders() : createShader failed to open geometry shader file\n");
		vkresult = VK_ERROR_INITIALIZATION_FAILED;
		return vkresult;
	}
	else
	{
		fprintf(gpFile, "createShaders() : createShader succeeded to open geometry shader file\n");
	}
	...
	if (size == 0)
	{
		fprintf(gpFile, "createShaders() : geometry shader file size is 0\n");
		vkresult = VK_ERROR_INITIALIZATION_FAILED;
		return vkresult;
	}
	...
	if (retVal != 1)
	{
		fprintf(gpFile, "createShaders() : failed to read geometry shader file\n");
		vkresult = VK_ERROR_INITIALIZATION_FAILED;
		return vkresult;
	}
	else
	{
		fprintf(gpFile, "createShaders() : succeeded to read geometry shader file\n");
	}
	...
	vkresult = vkCreateShaderModule(
					...
					NULL,
					&vkShaderModule_geometry_shader);

	if (vkresult != VK_SUCCESS)
	{
		fprintf(gpFile,
				"createShaders() : vkCreateShaderModule() failed for geometry shader (%d)\n",
				vkresult);
		return vkresult;
	}
	else
	...
	fprintf(gpFile, "Geometry Shader Module successfully created\n");
