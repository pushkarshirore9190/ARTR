...
-(void)keyDown:(NSEvent*)event
{
    ...
    switch(key)
    {
        // Fullscreen toggle (same as Windows ESC)
        case 27:
            [[self window] toggleFullScreen:self];
            bFullScreen = !bFullScreen;
            break;

        // Quit (same as Windows Q/q)
        case 'Q':
        case 'q':
...
-(VkResult) rebuildCommandBuffersIfReady
{
    ...
    // Now safe: only this (main) thread touches Vulkan
    if (vkDevice)
...
-(VkResult)createShaders
{
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        {
            fprintf(gpFile,
                    "createShaders() : Failed to open %s\n",
                    shaders[i].filename);
        ...
        if(retVal != 1)
        {
            ...
            fprintf(gpFile,
                    "createShaders() : Failed to read %s\n",
                    shaders[i].filename);
        ...
        {
            fprintf(gpFile,
                    "createShaders() : vkCreateShaderModule() failed for %s (%d)\n",
                    shaders[i].filename,
                    vkresult);
        ...
        fprintf(gpFile,
                "createShaders() : Successfully created shader module for %s\n",
                shaders[i].filename);
    }
