...
layout(location=3) in vec3 out_viwerVector;
...
void main(void)
{
    ...
    if(u_UniformData.LKeyIsPressed == 1)
    {
        ...
        vec3 diffuse1 = vec3(u_UniformData.lightDiffuse1) * vec3(u_UniformData.materialDiffuse) * max(dot(N, L1), 0.0);
        vec3 R1 = reflect(-L1, N);
        vec3 specular1 = vec3(u_UniformData.lightSpecular1) 
        ...
        vec3 diffuse2 = vec3(u_UniformData.lightDiffuse2) * vec3(u_UniformData.materialDiffuse) * max(dot(N, L2), 0.0);
        vec3 R2 = reflect(-L2, N);
        vec3 specular2 = vec3(u_UniformData.lightSpecular2) 
                       * vec3(u_UniformData.materialSpecular) 
                       * pow(max(dot(R2, V), 0.0), u_UniformData.materialShininess);

        // Final color = sum of both
        finalColor = ambient1 + diffuse1 + specular1 + ambient2 + diffuse2 + specular2;
    }
    else
    {
