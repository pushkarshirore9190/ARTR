...
void main(void)
{
	...
	if(u_UniformData.LKeyIsPressed == 1)
	{
		...
		vec3 ambientLight = vec3(u_UniformData.lightAmbient) * vec3(pushConstantData_Material.materialAmbient);
		vec3 diffuseLight = vec3(u_UniformData.lightDiffuse) * vec3(pushConstantData_Material.materialDiffuse) * max(dot(normalisedLightDirection, normalisedTransformedNormal),0.0);
		...
		vec3 specularLight = vec3(u_UniformData.lightSpecular) 
                   * vec3(pushConstantData_Material.materialSpecular) 
                   * pow(max(dot(reflectionVector, normalisedViewerVector), 0.0),pushConstantData_Material.materialShininess);
