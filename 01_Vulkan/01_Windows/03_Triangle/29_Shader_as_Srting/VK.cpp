...
// glsl - spriv related header files
#include <SPIRV/GlslangToSpv.h>
#include <Public/ShaderLang.h> // started needing recently
#include"VK.h"
...
// glslang/spriv related libraries
#pragma comment(lib,"glslang.lib") // started recently
#pragma comment(lib,"SPIRV-Tools.lib") // needed right from beginning
...
VkResult createShaders(void)
{
	...
	if (bDone == FALSE)
	{
		fprintf(gpFile, "createShaders() : glsl2spv() function failed for vertex shader.\n");
		vkresult = VK_ERROR_INITIALIZATION_FAILED; // return hardcoded failure
		return vkresult;
	}
	else
	...
	if (bDone == FALSE)  // fix typo: FLASE -> false
	{
		fprintf(gpFile, "createShaders() : glsl2spv() function failed for fragment shader.\n");
		vkresult = VK_ERROR_INITIALIZATION_FAILED; // return hardcoded failure
		return vkresult;
	}
	else
...
BOOL glsl2spv(const VkShaderStageFlagBits Shader_type, const char* pShader, vector <unsigned int>& Spirv)
{
	// function declaration
	...
	// find the shader langage in the form of EShLanguage
	EShLanguage language = findShaderLanguage(Shader_type);

	// create TShader type shader from above EShLanguage lanhguae
	glslang::TShader shader(language); // by calling tShader construtor
	...
	// initialise built in shader resources
	TBuiltInResource resources;

	initBuiltInResources(resources);

	// set our shader string in above array as tshaders string
	shader.setStrings(shaderStrings, 1);

	//specify ruless of shader compilation as spirv rules and vulkan rules
	...
	if (!shader.parse(&resources, 100, false, ruleMessages))
	{
		...
		fprintf(gpFile, "glsl2spv() : parse() failed with log:\n%s\n", shader.getInfoLog());
		fprintf(gpFile, "glsl2spv() : parse() failed with debug log:\n%s\n", shader.getInfoDebugLog());

		return FALSE;
	}
	else
	{
		fprintf(gpFile, "glsl2spv() : parse() succeeded.\n");
	}


	// add shader to T Programm
	...
	{
		fprintf(gpFile, "glsl2spv() : program.link() failed with log:\n");

		return FALSE;
	}
	else
	{
		fprintf(gpFile, "glsl2spv() : program.link() succeeded.\n");
	}

	// and now set this whole cooked programm and spv code to glslang library
